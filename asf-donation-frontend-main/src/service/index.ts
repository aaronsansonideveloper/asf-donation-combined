...
export const getStore = <T extends keyof typeof stores | [keyof typeof stores, string | undefined]>(
  storeName: T
): ReduxState[T extends keyof typeof stores ? T : T[0]] => {
  if (Array.isArray(storeName)) {
    if (storeName[1] && stores[storeName[0]].slice.branch?.includes(storeName[1])) {
      return reduxStore.getState()[
        `${storeName[0]}.${storeName[1]}` as T extends keyof typeof stores ? T : T[0]
      ];
    } else {
    ...
    return reduxStore.getState()[storeName as T extends keyof typeof stores ? T : T[0]];
  }
};
