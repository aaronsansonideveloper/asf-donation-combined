...
import { FieldSwitchProps } from '../components/form/field-switch';
import { FieldTextProps } from '../components/form/field-text-field';
import { InputType, getField } from '../utils/get-field';
import { FieldAutocompleteProps } from '../components/form/field-autocompleteGoogle';
...
export const useGetField = (methods: UseFormReturn | null = null) => {
  const useGetFieldByFormConfig = (keyBase: string, item: FormConfigItem) => {
    ...
    if (wrapper) {
      let Wrapper = wrapper;
      return (
        <Wrapper currentConfig={item} name={key} formMethods={methods} key={key}>
          {fieldNode}
        </Wrapper>
      );
    }
