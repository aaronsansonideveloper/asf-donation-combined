...
import React, { Fragment, PropsWithChildren, ReactElement, useMemo, useState } from 'react';
...
export function useFields(
    ...
    if (prefix) {
      ...
      keyArr.forEach((key, index) => {
        if (index != keyArr.length - 1) {
          ...
          if (required) {
            if (currentShape[key]) {
              currentShape[key] = currentShape[key].required('This is required');
            } else {
              currentShape[key] = Yup.string().required('This is required');
            }
          }
        }
      });
  ...
  let formNode = useMemo(() => {
    return (
      <FormProvider onSubmit={handleSubmit(onSubmit as any)} formRef={methods}>
